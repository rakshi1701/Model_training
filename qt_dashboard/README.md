...
| ☁️ **Kaggle Cloud GPU Training** | Credentials (import `kaggle.json`, type them, or **🚪 Disconnect account** to delete the stored token from this machine), weekly GPU-quota estimate, dataset staging + kernel dispatch with a live progress log, resume-from-capped-run picker, and the job board: status counts, filter/search, one card per job with model downloads, ingest, stop, log + GPU telemetry, and removal. Live polling re-checks every 30s and auto-ingests finished runs. |
